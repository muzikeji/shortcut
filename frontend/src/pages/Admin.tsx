...
export default function Admin() {
  const { user } = useAuth();
  const [tab, setTab] = useState<'users' | 'shortcuts' | 'settings' | 'update'>('users');
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-0.5 mb-6 w-fit">
        {(['users', 'shortcuts', 'settings', 'update'] as const).map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-md text-sm font-medium ${tab === t ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}
          >
            {{ users: '用户管理', shortcuts: '分享管理', settings: '站点设置', update: '系统升级' }[t]}
          </button>
        ))}
      </div>

      {tab === 'users' ? <UserManagement /> : tab === 'shortcuts' ? <ShortcutManagement /> : tab === 'settings' ? <SiteSettings /> : <UpdateSystem />}
    </div>
  );
}
...
function UpdateSystem() {
  ...
  const runUpdate = async () => {
    if (!confirm('升级将覆盖除 data/ 和 uploads/ 外的所有文件。是否继续？')) return;
    setUpdating(true);
    setErr('');
    setMsg('');
    setLog([]);

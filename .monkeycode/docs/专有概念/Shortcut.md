...
- 支持三种状态：`pending`（待审核）、`active`（活跃）、`removed`（已下架）
- 管理员发布直接 active，普通用户发布为 pending 待审核
...
|------|------|
| 数据库表 | `shortcuts` 表 |
| 后端路由 | `php-shortcut/src/routes/shortcuts.php` |
| 元数据抓取 | `fetchShortcutMeta()` + `PlistParser::parseShortcutInfo()` |
| 管理审核 | `php-shortcut/src/routes/admin.php` (`approveShortcut/rejectShortcut`) |
...
interface Shortcut {
  ...
  liked: boolean;          // 当前用户是否已点赞（当前用户查询结果）
  status: 'pending' | 'active' | 'removed'; // 状态
...
| 字段 | 类型 | 描述 | 约束 |
|------|------|------|------|
| `title` | `string` | 标题 | 必填；发布时被抓取的真实名称覆盖 |
| `file_url` | `string` | iCloud 链接 | 必填，需匹配 `icloud.com/shortcuts/[a-z0-9]+`，去重检查 |
| `category` | `string` | 分类 | 可选，默认 '其他' |
| `status` | `enum` | 状态 | `pending` / `active` / `removed`，取决于发布者角色 |
| `user_id` | `number` | 作者 | 外键，关联 users 表 |
| `slug` | `string` | 唯一标识 | 10 位秒级时间戳，UNIQUE，生成后固定 |
| `color` | `string` | 主题色 | `#RRGGBB`，抓取图标颜色；空时前端回退 `#3B82F6` |
| `stats` | `string` | 统计 JSON | 含 `actionCount`/`size`/`permissions`/`actionTypes` |

## 不变量

1. **URL 唯一性**: 同一 iCloud URL 不可被重复发布
2. **slug 唯一性**: 每个快捷指令的 slug 全局唯一，生成后固定不变
3. **状态流转**: pending → active（审核通过）或 pending → removed（审核拒绝），active ↔ removed（下架/恢复）
4. **编辑重置**: 修改非活跃状态的快捷指令会自动重置为 pending 并发送企微通知
5. **仅作者可删除**: `DELETE` 操作仅允许作者本人执行（管理员可删除，见管理接口）
6. **下载计数不递减**: `download_count` 只增不减

## 生命周期

```mermaid
stateDiagram-v2
    [*] --> Pending: POST /api/shortcuts (普通用户发布)
    [*] --> Active: POST /api/shortcuts (管理员/owner 发布)
    Pending --> Active: approve (管理员审核通过)
    Pending --> Removed: reject (管理员审核拒绝)
    Active --> Removed: PUT /:id/remove (下架)
    Removed --> Active: PUT /:id/restore (恢复)
    Active --> [*]: DELETE /:id (作者或管理员删除)
    Removed --> [*]: DELETE /:id (作者或管理员删除)
    Pending --> [*]: DELETE /:id (作者或管理员删除)
```
...
| 状态 | 描述 | 允许的转换 |
|------|------|-----------|
| `pending` | 待审核，不在首页显示，仅管理员可见 | → active（通过审核）, removed（拒绝审核）, deleted（删除） |
| `active` | 正常展示，可互动（点赞/评论） | → removed（下架）, deleted（删除） |
